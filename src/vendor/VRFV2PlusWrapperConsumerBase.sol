 ...
 * @dev PURPOSE
 *
 * @dev Create VRF V2+ requests without the need for subscription management. Rather than
 * creating
 * @dev and funding a VRF V2+ subscription, a user can use this wrapper to create one off
 * requests,
 * @dev paying up front rather than at fulfillment.
 *
 * @dev Since the price is determined using the gas price of the request transaction
 * rather than
 * @dev the fulfillment transaction, the wrapper charges an additional premium on callback
 * gas
 * @dev usage, in addition to some extra overhead costs associated with the VRFV2Wrapper
 * contract.
 * *****************************************************************************
 * @dev USAGE
 *
 * @dev Calling contracts must inherit from VRFV2PlusWrapperConsumerBase. The consumer
 * must be funded
 * @dev with enough LINK or ether to make the request, otherwise requests will revert. To
 * request randomness,
 * @dev call the 'requestRandomWords' function with the desired VRF parameters. This
 * function handles
 * @dev paying for the request based on the current pricing.
 *
 * @dev Consumers must implement the fullfillRandomWords function, which will be called
 * during
...
abstract contract VRFV2PlusWrapperConsumerBase {
  ...
  function _initVRFWrapper(address _vrfV2PlusWrapper) internal {
   ...
   *
   * @param _callbackGasLimit is the gas limit that should be used when calling the
   * consumer's
   *        fulfillRandomWords function.
   * @param _requestConfirmations is the number of confirmations to wait before fulfilling
   * the
   *        request. A higher number of confirmations increases security by reducing the
   * likelihood
  ...
  // chainlink-solidity/prefix-internal-functions-with-underscore
  ...
  function requestRandomnessPayInNative(
    ...
    requestPrice =
      i_vrfV2PlusWrapper.calculateRequestPriceNative(_callbackGasLimit, _numWords);
    return (
  ...
  /**
   * @notice fulfillRandomWords handles the VRF V2 wrapper response. The consuming
   * contract must
   * @notice implement it.
   *
